...
public class FileManager {
	...
	public String doFileUpload(byte[] bytes, String originalFilename, String path) throws Exception {
		...
		if(fileExt == null || "".equals(fileExt) || ".".equals(fileExt)) { // 확장자가 없는 경우 //ex) 강아지.
			return null;
		}
		...
		*/
		...
		fos.write(bytes); // 넘겨온 내용물 bytes를 파일에 넣어주는 것이다.
	...
	public boolean doFileDownload(String fileName, String originalFilename, String path, HttpServletResponse response) {
        ...
        // 해당경로에 \ 를 더하고 파일명을 더한 경로까지 나타내어준 파일명(문자열)을 만든다. 
		// pathname 은 예를 들면, C:\NCS\workspace_spring_framework\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\board\resources\files\2024062809392389156603107400.png 이다.   
